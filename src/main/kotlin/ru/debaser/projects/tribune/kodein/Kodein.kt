...
fun Kodein.MainBuilder.binds(app: Application) {
    constant(tag = "password") with (
            app.environment.config.propertyOrNull("tribune.secret.password")?.getString() ?:
                throw ConfigurationException("Password is not specified")
            )
    constant(tag = "salt") with (
            app.environment.config.propertyOrNull("tribune.secret.salt")?.getString() ?:
                throw ConfigurationException("Salt is not specified")
            )
    constant(tag = "jwt-secret-path") with (
            app.environment.config.propertyOrNull("tribune.jwt.jwt-secret-path")?.getString() ?:
